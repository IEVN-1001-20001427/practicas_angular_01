...
@Component({
  selector: 'app-examen-p2',
  templateUrl: './examen-p2.component.html',
  styleUrls: ['./examen-p2.component.css']
})
export class ExamenP2Component implements OnInit {
  examen!:FormGroup;
  saludo:string = '';
  res:string = '';
  img:string = '';
  ...
  onSubmit(){
    this.saludo = 'Hola '+this.sx()+' '+this.examen.controls['nombre'].value+' '
    +this.examen.controls['apaterno'].value+' '
    +this.examen.controls['amaterno'].value+' tienes '
  ...
  sx(): string{
    return this.examen.controls['sexo'].value == 'M' ? 'Sr.' : 'Sra.';
  }

  calcedad(): string{
    var bday: Date = new Date(this.examen.controls['yyyy'].value,this.examen.controls['mm'].value,this.examen.controls['dd'].value);
  ...
  signo(): string{
    ...
    arr.forEach(element => {
      if(element.yrs.includes(this.examen.controls['yyyy'].value)){
        sn=element.id;
      }
    });
  ...
  prueba(): string{
    var answ:number = 0;
    if(this.examen.controls['p1'].value == 'a'){answ++;}
    if(this.examen.controls['p2'].value == 'c'){answ++;}
    if(this.examen.controls['p3'].value == 'b'){answ++;}
    if(this.examen.controls['p4'].value == 'd'){answ++;}
    if(this.examen.controls['p5'].value == 'a'){answ++;}
    answ = answ*2;
